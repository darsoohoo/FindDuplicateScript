...
<#
.SYNOPSIS
Finds duplicate record_id values within and across CSV files.
.EXAMPLE
.\Find-DuplicateRecordIds.ps1 -FolderPath 'C:\Data\CsvFiles'
.EXAMPLE
.\Find-DuplicateRecordIds.ps1 -FolderPath 'C:\Data\CsvFiles' -Recurse -IgnoreCase -OutputPath 'C:\Reports\duplicates.csv'
.NOTES
Matching is exact and case-sensitive by default; whitespace is preserved.
Blank record_id values are skipped. DataRow is the CSV record number,
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $folder.FullName 'duplicate-record-ids.csv'
}
...
foreach ($file in $files) {
    ...
    Import-Csv -LiteralPath $file.FullName -Delimiter $Delimiter -Encoding UTF8 | ForEach-Object {
        $dataRow++
        $totalRows++
        if ($dataRow -eq 1 -and $_.PSObject.Properties.Name -notcontains 'record_id') {
            throw "CSV is missing the record_id column: $($file.FullName)"
        }
        ...
        if ([string]::IsNullOrWhiteSpace($recordId)) {
            ...
            if (-not $seen.ContainsKey($recordId)) {
                $seen.Add($recordId, [System.Collections.Generic.List[object]]::new())
            }
            $seen[$recordId].Add($occurrence)
        }
        if (($elapsed.Elapsed.TotalSeconds - $lastProgressSeconds) -ge $ProgressIntervalSeconds) {
            Write-Host "[$(Get-Date -Format 'HH:mm:ss')] File $fileNumber/$($files.Count): $dataRow rows read; total $totalRows rows; $($seen.Count) unique record IDs; $blankRows blank record IDs skipped; elapsed $($elapsed.Elapsed.ToString('hh\:mm\:ss'))."
...
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Building duplicate report from $($seen.Count) unique record IDs..."
$duplicateGroups = 0L
$duplicateRows = 0L
$report = @(
...
if ($report.Count -gt 0) {
    ...
    '"record_id","OccurrenceCount","SourceFile","DataRow"' |
...
Write-Host "Scanned $($files.Count) CSV files and $totalRows data rows; skipped $blankRows blank record IDs."
Write-Host "Found $duplicateGroups duplicated record IDs across $duplicateRows rows."
