...
- The remote-path report lists every matching occurrence with its originating `record_id`, total occurrence count, full source filename, and zero-based `DataRow` (0 is the first record after the header). IDs are preserved as text, including leading zeros; missing or blank IDs produce empty fields. Quoted multiline fields count as one record. The separate record-ID duplicate script still uses one-based `DataRow` values.
